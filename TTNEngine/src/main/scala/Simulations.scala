...
object Simulations {
  ...
  : Unit = {
    ...
    if (num_games == 1) {
      val result = computer_vs_computer(state, agent_1, agent_2)
      write_result(result, file_path = s"${file_path}_0.json")
    } else {
  ...
  def run_EqualTimeSimulations(): Unit = {
    for (d <- 5 to 12) {
      for (t <- 1 to 4) {
        val time: Double = t * 2
        run_experiment("EqualTimeSimulations", dim = d, time, time, num_games = 5, connect_num = 5)
      }
    }
  }

  def EffectivenessSimulation(): Unit = {
    for (d <- 5 to 12) {
      for (t <- 1 to 5) {
        run_experiment("EffectivenessSimulation", dim = d, t*2, 0.1, num_games = 10, connect_num = 5)
      }
    }
  }

  def AbnormalPointTest(): Unit = {
    run_experiment("AbnormalPointTest", 7, 8, 4, 100, connect_num = 4)
  }

  def main(args: Array[String]): Unit = {
    //run_EqualTimeSimulations()
    //run_experiment("EqualTimeSimulations", dim = 12, 8, 8, num_games = 1, connect_num = 5)
    AbnormalPointTest()
  }
}